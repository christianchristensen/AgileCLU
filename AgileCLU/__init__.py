...
def e_pw_hash(str, username, proto, hostname, basepath):
    hash = base64.b64encode(
        pyDes.triple_des(
            epwbasekey(username, proto, hostname, basepath)
        ).encrypt(str, padmode=2))
    return hash
...
class AgileCLU:
    ...
    def __init__(self, profile='default'):
        ...
        upw = e_pw_dehash(cfg.get("Identity", "password"), self.uid,
                          self.egress_protocol, self.egress_hostname,
        ...
        if self.token is None:
            ...
            logger.critical(self.uid +
                            " login failed - check account credentials")
            sys.exit(1)

        logger.info(self.uid + " " + self.token + ", login " + str(self.user) +
                    " tokenized")
    ...
    def mapperurlstr(self):
        logger.info(self.uid + " " + self.token + ", mapperurl=" +
                    self.mapperurl)
        return self.mapperurl

    def cacheurlstr(self):
        logger.info(self.uid + " " + self.token + ", cacheurl=" +
                    self.cacheurl)
        return self.cacheurl

    def postmultiurlstr(self):
        logger.info(self.uid + " " + self.token + ", postmultiurl=" +
                    self.postmultiurl)
    ...
    def stat(self, path):
        ...
        logger.info(self.uid + " " + self.token + ", stat " + path + " = " +
                    str(r))
        return r

    def logout(self):
        r = self.api.logout(self.token)
        logger.info(self.uid + " " + self.token + ", logout = " + str(r) +
                    ", detokenized")
        return r
    ...
    def listDir(self, path, pageSize=10000, cookie=0, stat=True):
        ...
        logger.info(self.uid + " " + self.token + ", listDir " + path +
                    " pageSize " + str(pageSize) + " cookie " + str(cookie) +
                    " stat " + str(stat))
        #  + " = " + str(r))
        return r
    ...
    def listFile(self, path, pageSize=10000, cookie=0, stat=True):
        ...
        logger.info(self.uid + " " + self.token + ", listFile " + path +
                    " pageSize " + str(pageSize) + " cookie " + str(cookie) +
                    " stat " + str(stat))
        #  + " = " + str(r))
        return r
    ...
    def createMultipartPiece(self, mpid='', number=0, size=0, checksum='',
                             uri=''):
        r = self.api.createMultipartPiece(self.token, mpid, number, size,
                                          checksum, uri)
        logger.info(self.uid + " " + self.token +
                    ", createMultipartPiece mpid " + mpid + " number " +
                    str(number) + " size " + str(size) + " checksum " +
                    checksum + " uri " + uri)

    def completeMultipart(self, mpid=''):
        r = self.api.completeMultipart(self.token, mpid)
        logger.info(self.uid + " " + self.token + ", completeMultipart " +
                    mpid)
        return r

    def getMultipartStatus(self, mpid=''):
        r = self.api.getMultipartStatus(self.token, mpid)
        logger.info(self.uid + " " + self.token + ", getMultipartStatus " +
                    mpid)
        return r
    ...
    def listMultipartPiece(self, mpid='', lastpiece=0, pagesize=100):
        ...
        logger.info(self.uid + " " + self.token + ", listMultipartPiece " +
                    mpid + " lastpiece " + str(lastpiece) + " pagesize " +
                    str(pagesize))
        return r

    def makeDir(self, path):
        r = self.api.makeDir(self.token, path)
        logger.info(self.uid + " " + self.token + ", makeDir " + path + " = "
                    + str(r))
        return r

    def makeDir2(self, path):
        r = self.api.makeDir2(self.token, path)
        logger.info(self.uid + " " + self.token + ", makeDir2 " + path + " = "
                    + str(r))
        return r

    def deleteFile(self, path):
        r = self.api.deleteFile(self.token, path)
        logger.info(self.uid + " " + self.token + ", deleteFile " + path +
                    " = " + str(r))
        return r
    ...
    def rm(self, path):
        if (self.fexists(path)):
            ...
            if (r == 0):
                logger.info(self.uid + " " + self.token + ", rm " + path +
                            " succeeded")
                return True
            else:
                logger.warning(self.uid + " " + self.token + ", rm " + path +
                               " failed")
                return False
        else:
            logger.warning(self.uid + " " + self.token + ", rm " + path +
                           " skipped nonexistent file")

    def deleteDir(self, path):
        r = self.api.deleteDir(self.token, path)
        logger.info(self.uid + " " + self.token + ", deleteDir " + path +
                    " = " + str(r))
        return r

    def deleteObject(self, path):
        r = self.api.deleteObject(self.token, path)
        logger.info(self.uid + " " + self.token + ", deleteObject " + path +
                    " = " + str(r))
        return r

    def rename(self, path, newpath):
        r = self.api.rename(self.token, path, newpath)
        logger.info(self.uid + " " + self.token + ", rename " + path +
                    " to " + newpath + " = " + str(r))
        return r

    def copyFile(self, path, newpath):
        r = self.api.copyFile(self.token, path, newpath)
        logger.info(self.uid + " " + self.token + ", copyFile " + path +
                    " to " + newpath + " = " + str(r))
        return r

    def registerCallback(self, uri, flags=0, threshold=0):
        r = self.api.registerCallback(self.token, uri, flags, threshold)
        logger.info(self.uid + " " + self.token + ", registerCallback " + uri
                    + " flags " + str(flags) + " threshold " + str(threshold) +
                    " = " + str(r))
        return r
    ...
    def fetchFileHTTP(self, path, uri, username=None, password=None,
                      auth=None, callbackid=0, priority=0, flags=0,
                      expose_egress='COMPLETE'):
        r = self.api.fetchFileHTTP(self.token, path, uri, username, password,
                                   auth, callbackid, priority, flags,
                                   expose_egress)
        logger.info(self.uid + " " + self.token + ", fetchFileHTTP path " +
                    path + " uri " + uri + " username " + str(username) +
                    " auth " + str(auth) + " callbackid " + str(callbackid) +
                    " priority " + str(priority) + " flags " + str(flags) +
                    " expose_egress " + expose_egress + " = " + str(r))
        return r

    def fetchFileFTP(self, path, hostname, filename, username=None,
                     password=None, port=21, passive=True, callbackid=0,
                     priority=0, flags=0, expose_egress='COMPLETE'):
        r = self.api.fetchFileFTP(self.token, path, hostname, filename,
                                  username, password, port, passive,
                                  callbackid, priority, flags, expose_egress)
        logger.info(self.uid + " " + self.token + ", fetchFileFTP path " +
                    path + " hostname " + hostname + " filename " + filename
                    + " username " + str(username) + " port " + str(port) +
                    " passive " + str(passive) + " callbackid " +
                    str(callbackid) + " priority " + str(priority) + " flags "
                    + str(flags) + " expose_egress " + expose_egress + " = "
                    + str(r))
        return r

    def fexists(self, path):
        r = self.stat(path)
        logger.info(self.uid + " " + self.token + ", fexists " + path +
                    " = " + str(r))
        if ((r['code'] == 0) and (r['type'] == 2)):
            return True
        else:
            return False
    ...
    def dexists(self, path):
        r = self.stat(path)
        logger.info(self.uid + " " + self.token + ", dexists " + path +
                    " = " + str(r))
        if ((r['code'] == 0) and (r['type'] == 1)):
            return True
        else:
            return False
    ...
    def exists(self, path):
        ...
        #  + " = " + str(r))
        if (self.fexists(path) or self.dexists(path)):
            return True
        else:
            return False
    ...
    def read(self, path):
        logger.info(self.uid + " " + self.token + ", read " +
                    self.mapper.url + urllib2.quote(path))
        if (self.fexists(path)):
    ...
    def pbar_callback(self, param, current, total):
        if (self.pbar is None):
            widgets = [unicode(self.pbarfname,
    ...
    def post(self, sourceObject, targetPath, rename=None, mimetype='auto',
             mtime=None, egress_policy='COMPLETE', mkdir=False, callback=None):
        logger.info(self.uid + " " + self.token + ", post " + sourceObject +
                    " " + targetPath + ", rename=" + str(rename) +
                    ", mimetype=" + str(mimetype) + ", mtime=" + str(mtime) +
                    ", egress=" + str(egress_policy) + ", mkdir=" +
                    str(mkdir))
        if (not os.path.isfile(sourceObject)):
            logger.info("local(" + sourceObject + ") does not exist")
            return False
        if (not self.dexists(targetPath)):
            logger.info("remote(" + targetPath + ") does not exist")
            return False
